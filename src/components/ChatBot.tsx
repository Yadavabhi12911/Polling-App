...
export default function ChatBot() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "model", text: "Let's create a poll! What's your question?" },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await chatWithPollBot(userText, fileInfo);
      ...
      if (res.type === "text") {
        ...
        const resultsText = res.data.length === 0 
          ? "No active polls found at the moment."
          : `📊 Poll Results Summary:\n\n${res.data.map((poll: any) => 
              `**${poll.question}**\nTotal Votes: ${poll.total_votes}\n${poll.options.map((opt: any) => 
                `• ${opt.text}: ${opt.votes} votes (${poll.total_votes > 0 ? ((opt.votes / poll.total_votes) * 100).toFixed(1) : 0}%)`
              ).join('\n')}\n`
            ).join('\n')}`;
  ...
  const handleReset = () => {
    resetChat();
    setMessages([
      { role: "model", text: "Let's create a new poll! What's your question?" },
    ]);
    setFilePreview(null);
    setFileType(null);
    setSelectedFile(null);
   ...
   return (
    ...
    <Card >
      <CardHeader>
        <CardTitle >Poll Creation Chatbot</CardTitle>
      </CardHeader>
